...
*@FileName : InvoiceMgmtDBDAOSearchInvoiceDetailRSQL.java
*@FileTitle : 
*Open Issues :
*Change history :
...
public class InvoiceMgmtDBDAOSearchInvoiceDetailRSQL implements ISQLTemplate{
	...
	/**
	  * <pre>
	  * List invoice detail items
	  * </pre>
	  */
	public InvoiceMgmtDBDAOSearchInvoiceDetailRSQL(){
		setQuery();
		...
		query.append("FileName : InvoiceMgmtDBDAOSearchInvoiceDetailRSQL").append("\n"); 
		query.append("*/").append("\n"); 
	}
	...
	public void setQuery(){
		...
		query.append("     ,SUM(REV_ACT_AMT) AS INV_REV_ACT_AMT" ).append("\n"); 
		query.append("     ,SUM(EXP_ACT_AMT) AS INV_EXP_ACT_AMT" ).append("\n"); 
		...
		query.append("		AND INV.ACCT_YRMON >= REPLACE(@[cre_dt_fm], '-','')" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if(${cre_dt_to} != '')" ).append("\n"); 
		query.append("		AND INV.ACCT_YRMON <= REPLACE(@[cre_dt_to], '-','')" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		query.append("	#if (${jo_crr_cd}!=''  && ${jo_crr_cd} != 'ALL')" ).append("\n"); 
		query.append("        AND INV.JO_CRR_CD IN (#foreach($key IN ${jo_crr_cd_list})#if($velocityCount < $jo_crr_cd_list.size()) '$key', #else '$key' #end #end)" ).append("\n"); 
		query.append("	#end" ).append("\n"); 
		...
		query.append(" WHERE 1=1" ).append("\n"); 
		query.append(" GROUP BY GROUPING SETS ((JO_CRR_CD, RLANE_CD, CSR_NO, APRO_FLG, REV_EXP, ITEM, CUST_VNDR_CNT_CD, CUST_VNDR_SEQ, PRNR_REF_NO, CUST_VNDR_ENG_NM, LOCL_CURR_CD, INV_NO)," ).append("\n"); 
